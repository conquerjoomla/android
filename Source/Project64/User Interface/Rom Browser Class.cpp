...
void CRomBrowser::FillRomExtensionInfo(ROM_INFO * pRomInfo)
{
	...
	pRomInfo->TextColor = (std::strtol(String, 0, 16) & 0xFFFFFF);
	...
	int selcol = std::strtol(String, 0, 16);
	if (selcol < 0)
	{
		pRomInfo->SelColor = -1;
	}
	...
	{
		selcol = (std::strtol(String, 0, 16) & 0xFFFFFF);
	...
	pRomInfo->SelTextColor = (std::strtol(String, 0, 16) & 0xFFFFFF);
		...
		if (Extension == "7z")
		{
			try
			{
				...
				for (int i = 0; i < ZipFile.NumFiles(); i++)
				{
					...
					for (int x = 0; x < 0x40; x += 4)
					{
						*((DWORD *)&RomData[x]) = std::strtol(&szHeader[x * 2], 0, 16);
					}
