...
int CALLBACK CCheatsUI::CheatsCodeQuantProc(HWND hDlg, uint32_t uMsg, uint32_t wParam, uint32_t lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		Start = (uint16_t)(Range.c_str()[0] == '$' ? std::strtoll(&Range.c_str()[1], 0, 16) : atol(Range.c_str()));
		const char * ReadPos = strrchr(Range.c_str(), '-');
		if (ReadPos != NULL)
		{
			Stop = (uint16_t)(ReadPos[1] == '$' ? std::strtoll(&ReadPos[2], 0, 16) : atol(&ReadPos[1]));
		}
		else
		{
			Stop = 0;
	...
	case WM_COMMAND:
		...
		case IDC_VALUE:
			if (HIWORD(wParam) == EN_UPDATE)
			{
				...
				Value = szTmp[0] == '$' ? std::strtoll(&szTmp[1], 0, 16) : std::strtoll(szTmp, 0, 16);
