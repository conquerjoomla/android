...
bool CCheats::LoadCode(int CheatNo, const char * CheatString)
{
	...
	while (ReadPos)
	{
		...
		CodeEntry.Command = std::strtoll(ReadPos, 0, 16);
		...
		if (strncmp(ReadPos, "????", 4) == 0)
		{
			...
			CodeEntry.Value = CheatExt[0] == '$' ? (uint16_t)std::strtoll(&CheatExt.c_str()[1], 0, 16) : (uint16_t)atol(CheatExt.c_str());
		}
		else if (strncmp(ReadPos, "??", 2) == 0)
		{
			...
			stdstr CheatExt = g_Settings->LoadStringIndex(Cheat_Extension, CheatNo);
			if (CheatExt.empty()) { return false; }
			CodeEntry.Value = (uint8_t)(std::strtoll(ReadPos, 0, 16));
			CodeEntry.Value |= (CheatExt[0] == '$' ? (uint8_t)std::strtoll(&CheatExt.c_str()[1], 0, 16) : (uint8_t)atol(CheatExt.c_str())) << 16;
		}
		else if (strncmp(&ReadPos[2], "??", 2) == 0)
		{
			...
			stdstr CheatExt = g_Settings->LoadStringIndex(Cheat_Extension, CheatNo);
			if (CheatExt.empty()) { return false; }
			CodeEntry.Value = (uint16_t)(std::strtoll(ReadPos, 0, 16) << 16);
			CodeEntry.Value |= CheatExt[0] == '$' ? (uint8_t)std::strtoll(&CheatExt.c_str()[1], 0, 16) : (uint8_t)atol(CheatExt.c_str());
		}
		else
		{
			CodeEntry.Value = (uint16_t)std::strtoll(ReadPos, 0, 16);
		}
