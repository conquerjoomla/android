...
void CX86Ops::X86BreakPoint (const wchar_t * FileName, int LineNumber)
{
    stdstr file_name;
    file_name.FromUTF16(FileName);

    Pushad();
    PushImm32(stdstr_f("%d",LineNumber).c_str(),LineNumber);
    PushImm32(file_name.c_str(),(uint32_t)FileName);
...
void CX86Ops::Call_Direct(void * FunctAddress, const char * FunctName)
{
    ...
    PUTDST32(m_RecompPos,(uint32_t)FunctAddress-(uint32_t)m_RecompPos - 4);
}
